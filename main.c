...
void  process_cmds()
{
	...
	uint16_t data_2 = strlen(tmp);
		...
		if(!strncmp(cmd,"+IPD",4))
		{
			...
			_delay_ms(150);
			tmp[0]=0;
			strcat_P(tmp,web_page_title);
			uart_puts(tmp);
		
			tmp[0]=0;
			strcat_P(tmp,web_page_body1);
			uart_puts(tmp);
			tmp[0]=0;
			strcat_P(tmp,web_page_body2);
			uart_puts(tmp);
			//snprintf(tmp,sizeof(tmp),"%0.2f D Celcius",temp);
			//snprintf(tmp,sizeof(tmp),"%d ADC data.",data);
			snprintf(tmp,sizeof(tmp),"%.2f Celcius</p>",temp);
			uart_puts(tmp);
			//snprintf(tmp,sizeof(tmp),"Potentiometer = %.2f Volts",pot);
			//uart_puts(tmp);
			tmp[0]=0;
			strcat_P(tmp,web_page_body3);
			uart_puts(tmp);
			snprintf(tmp,sizeof(tmp),"%.2f Volts</p>",pot);
			uart_puts(tmp);
			tmp[0]=0;
			strcat_P(tmp,web_page_body4);
			uart_puts(tmp);
			snprintf(tmp,sizeof(tmp),"%s </p>",ip);
			uart_puts(tmp);
			tmp[0]=0;
			strcat_P(tmp,web_page_end);
			uart_puts(tmp);
	  		//uart_puts("<html><head><title>WIFI Home Page</title></head>");
  		//_delay_ms(500);
			//uart_puts("<body>Hello World</body></html>");
	  		_delay_ms(500);
